...
def calculate_rapidity(particle):
    particle_y = 1 / 2 * np.log((particle.energy + particle.pz) / (particle.energy - particle.pz))
...
def calculate_rapidity_for_jax(inputs):
    ...
    pz = inputs[2]
    energy = inputs[3]
    y = 1 / 2 * jax.numpy.log((energy + pz) / (energy - pz))
    particle_y = jax.numpy.nan_to_num(y, nan=EMPTY_FLOAT)
    return particle_y
...
def calculate_constraint_term(
    ...
    constr_term = (
        ...
        / 2
        * (
            ((corrected_distr1 - uncorr_distr1 - mean_correction1) / (sigma_correction1)) ** 2
...
def return_deviations_where_necessary(m_1_sq, m_2_sq, both_false_mask):
    ...
    deviations = (
        ak.fill_none(ak.mask(m_1_sq_masked, is_bigger1), 0)
        - m_tw_sq_arr_1
        + ak.fill_none(ak.mask(m_2_sq_masked, is_bigger2), 0)
        - m_tw_sq_arr_2
    )
    return deviations
...
def boost_a_cm_of_b(inputs, mass_b):
    ...
    boosted_p = (
        p_vec_a
        + (gamma_b - 1) * (jax.numpy.linalg.vecdot(p_vec_a, normal_n, axis=0)) * normal_n
        - gamma_b * norm_v * a_energy * normal_n
    )
    return jax.numpy.stack(
        [
            boosted_p[0],
...
def calculate_b_corrected(inputs, mean_sigma_correction_array, calculate_constr_term=False):
    ...
    m_bb_rec = calculate_invariant_mass(jax.numpy.concatenate([b1, b2], axis=0))
...
def calculate_tau_corrected(inputs, mean_sigma_correction_array, calculate_constr_term=False):
    ...
    m_tautau = 125
    m_tautau_rec = calculate_invariant_mass(jax.numpy.concatenate([tau1, tau2], axis=0))
    m_tautau_rec = jax.numpy.nan_to_num(m_tautau_rec, nan=EMPTY_FLOAT)
...
def calculate_t_vis(inputs):
    ...
    def true_fun_level2(operands):
        ...
        diff_arr1 = jax.numpy.nan_to_num(diff_arr1, nan=EMPTY_FLOAT)
        ...
        diff_arr2 = jax.numpy.nan_to_num(diff_arr2, nan=EMPTY_FLOAT)
...
def calculate_cos_theta_cms_h1_b1(inputs):
    ...
    # Boost b1 in cms of H_bb, take angle between the two
...
# 2 boost version
def calculate_cos_theta_cms_h1_b1_2boosts(inputs):
    """Calculates the angle between the b1 in the cms of the H_bb and the H_bb in HH cms
    Args:
        inputs: Array of the b and tau four-vectors in detector space. Order is particle 1 pt, eta, phi, mass,
                then particle 2, 3, 4, where particles 1 & 2 are b1 & b2, and 3 & 4 are tau1 & tau2
    Returns:
        cos_theta_cms_h1_b1: Angle between the two vectors
    """
    b1_corrected = det_coords_to_fourmomentum(inputs[:4])  # four-momentum
    h1 = calculate_hbb(inputs)  # four-momentum
    dihiggs_system = calculate_dihiggs_system(inputs)
    dihiggs_system_detspace = fourmomentum_to_det_coord(dihiggs_system)
    # Boost H_bb in HH cms
    ...
    # Boost b_1 in HH cms
    b1_cms_dihiggs = boost_a_cm_of_b(
        jax.numpy.concatenate([b1_corrected, dihiggs_system], axis=0), dihiggs_system_detspace[3]
    )
    # Boost b1 from HH cms to H_bb cms
    b_cms_h1 = boost_a_cm_of_b(
        jax.numpy.concatenate([b1_cms_dihiggs, h1_cms_dihiggs], axis=0), h1_cms_dihiggs_detspace[3]
    )
    # Calculate the angle
    cos_theta_cms_h1_b1 = signed_cos_deltaangle_for_jax(
        jax.numpy.concatenate([b_cms_h1[:3], h1_cms_dihiggs[:3]], axis=0)
    )
    return cos_theta_cms_h1_b1
...
# 2 boost version
def calculate_cos_theta_cms_h2_tau_vis1_2boosts(inputs):
    """Calculates the angle between the tau1 in the cms of the H_tautau and the H_tautau in det space
    Args:
        inputs: Array of the b and tau four-vectors in detector space. Order is particle 1 pt, eta, phi, mass,
                then particle 2, 3, 4, where particles 1 & 2 are b1 & b2, and 3 & 4 are tau1 & tau2
    Returns:
        cos_theta_cms_h2_tau_vis1: Angle between the two vectors, but in HH cms
    """
    tau1_corrected = det_coords_to_fourmomentum(inputs[8:12])  # four-momentum
    h2 = calculate_htautau(inputs)  # four-momentum
    dihiggs_system = calculate_dihiggs_system(inputs)
    dihiggs_system_detspace = fourmomentum_to_det_coord(dihiggs_system)
    # Boost H_tautau in HH cms
    h2_cms_dihiggs = boost_a_cm_of_b(jax.numpy.concatenate([h2, dihiggs_system], axis=0), dihiggs_system_detspace[3])
    h2_cms_dihiggs_detspace = fourmomentum_to_det_coord(h2_cms_dihiggs)
    # Boost tau1 in HH cms
    tau1_cms_dihiggs = boost_a_cm_of_b(
        jax.numpy.concatenate([tau1_corrected, dihiggs_system], axis=0), dihiggs_system_detspace[3]
    )
    # Boost tau 1 from HH cms to H_tautau cms
    tau_cms_h2 = boost_a_cm_of_b(
        jax.numpy.concatenate([tau1_cms_dihiggs, h2_cms_dihiggs], axis=0), h2_cms_dihiggs_detspace[3]
    )
    # Calculate the angle
    cos_theta_cms_h2_tau_vis1 = signed_cos_deltaangle_for_jax(
        jax.numpy.concatenate([tau_cms_h2[:3], h2_cms_dihiggs[:3]], axis=0)
    )
    return cos_theta_cms_h2_tau_vis1
...
def calculate_cos_theta_h1(inputs):
    ...
    dihiggs_system_detspace = fourmomentum_to_det_coord(dihiggs_system)
    h1_cms_dihiggs = boost_a_cm_of_b(jax.numpy.concatenate([h1, dihiggs_system], axis=0), dihiggs_system_detspace[3])
    cos_theta_h1 = signed_cos_deltaangle_for_jax(
        jax.numpy.concatenate([h1_cms_dihiggs[:3], dihiggs_system[:3]], axis=0)
    )
    return cos_theta_h1
...
def calculate_mean_correction(inputs, ch_id_mask):
    ...
    m_inv_rec = calculate_invariant_mass(jax.numpy.concatenate([part1, part2], axis=0))
...
def calculate_t_vis_y_diff(inputs):
    ...
    t_vis_y_diff = calculate_rapidity_for_jax(t1_vis_cms_tt_vis) - calculate_rapidity_for_jax(t2_vis_cms_tt_vis)
    return t_vis_y_diff
...
def calculate_tau1_cos_theta_star_cms_t1_vis_2boost(inputs):
    """Calculates the angle between tau1 in t_vis1 cms and t_vis1 in det. cms
    Args:
        inputs: Array of the b and tau four-vectors in detector space. Order is particle 1 phi, eta, phi, mass,
                then particle 2, 3, 4, where particles 1 & 2 are b1 & b2, and 3 & 4 are tau1 & tau2
    Returns:
        tau1_cos_theta_star_cms_t1_vis: angle between tau1 in t_vis1 cms and t_vis1 in det. cms
    """
    # Get relevant particles and quantities
    tau1 = det_coords_to_fourmomentum(inputs[8:12])  # four-momentum
    t1_vis = calculate_t_vis(inputs)[:4]  # four-momentum
    tt_vis_system = calculate_tt_vis_system(inputs)  # four-momentum
    tt_vis_system_mass = fourmomentum_to_det_coord(tt_vis_system)[3]
    # Boost t_vis and tau in tt cms
    t1_vis_cms_tt_vis = boost_a_cm_of_b(
        jax.numpy.concatenate([t1_vis, tt_vis_system]),
        tt_vis_system_mass,
    )
    tau1_cms_tt_vis = boost_a_cm_of_b(
        jax.numpy.concatenate([tau1, tt_vis_system]),
        tt_vis_system_mass,
    )
    t1_vis_mass = fourmomentum_to_det_coord(t1_vis_cms_tt_vis)[3]
    # Boost tau in t_vis cms
    tau1_cms_t1_vis = boost_a_cm_of_b(
        jax.numpy.concatenate([tau1_cms_tt_vis, t1_vis_cms_tt_vis]),
        t1_vis_mass,
    )
    tau1_cos_theta_star_cms_t1_vis = signed_cos_deltaangle_for_jax(
        jax.numpy.concatenate([tau1_cms_t1_vis[:3], t1_vis_cms_tt_vis[:3]], axis=0),
    )
    return tau1_cos_theta_star_cms_t1_vis
...
def calculate_tau2_cos_theta_star_cms_t2_vis_2boost(inputs):
    """Calculates the angle between tau2 in t_vis2 cms and t_vis2 in det. cms
    Args:
        inputs: Array of the b and tau four-vectors in detector space. Order is particle 1 phi, eta, phi, mass,
                then particle 2, 3, 4, where particles 1 & 2 are b1 & b2, and 3 & 4 are tau1 & tau2
    ...
    """
    # Get relevant particles and quantities
    tau2 = det_coords_to_fourmomentum(inputs[12:])  # four-momentum
    t2_vis = calculate_t_vis(inputs)[4:]  # four-momentum
    tt_vis_system = calculate_tt_vis_system(inputs)  # four-momentum
    tt_vis_system_mass = fourmomentum_to_det_coord(tt_vis_system)[3]
    # Boost t_vis and tau in tt cms
    t2_vis_cms_tt_vis = boost_a_cm_of_b(
    ...
    tau2_cms_tt_vis = boost_a_cm_of_b(
        jax.numpy.concatenate([tau2, tt_vis_system]),
        tt_vis_system_mass,
    )
    t2_vis_mass = fourmomentum_to_det_coord(t2_vis_cms_tt_vis)[3]
    # Boost tau in t_vis cms
    tau2_cms_t2_vis = boost_a_cm_of_b(
        jax.numpy.concatenate([tau2_cms_tt_vis, t2_vis_cms_tt_vis]),
        t2_vis_mass,
    )
    tau2_cos_theta_star_cms_t2_vis = signed_cos_deltaangle_for_jax(
        jax.numpy.concatenate([tau2_cms_t2_vis[:3], t2_vis_cms_tt_vis[:3]], axis=0),
    )
    return tau2_cos_theta_star_cms_t2_vis
...
def calculate_tau1_cos_theta_star_cms_wplus(inputs):
    ...
    tau1_cos_theta_star_cms_wplus = 2 * m_lb1**2 / (175**2 - 80.3**2 - 4.19**2) - 1
...
def calculate_tau2_cos_theta_star_cms_wminus(inputs):
    ...
    tau2_cos_theta_star_cms_wminus = 2 * m_lb2**2 / (175**2 - 80.3**2 - 4.19**2) - 1
    return tau2_cos_theta_star_cms_wminus


@producer(
    uses={"HHBJet", "channel_id", "Tau.*"},
    produces={"pdf_input_vars_reco_higgs.*"},
)
def create_pdf_input_vars_reco_higgs(
    self: Producer,
    events: ak.Array,
    nn_outputs: bool = False,
    inputs_inputs: bool = False,
    **kwargs,
) -> ak.Array:
    """
    ...
    Args:
        nn_outputs (bool): If the output masking should be such that UniteColumns can process the outputs.
            May not work with histogram creation / evaluation (not tested yet)
        inputs_inputs (bool): If the output should be not the hard scattering observables, but their inputs
    """
    ...
    tau_pt = ak.to_numpy(
        ak.fill_none(ak.pad_none(sorted_taus.pt, 2, axis=1, clip=True), EMPTY_FLOAT),
        allow_missing=False,
    )
    tau_eta = ak.to_numpy(
        ak.fill_none(ak.pad_none(sorted_taus.eta, 2, axis=1, clip=True), EMPTY_FLOAT),
        allow_missing=False,
    )
    tau_phi = ak.to_numpy(
        ak.fill_none(ak.pad_none(sorted_taus.phi, 2, axis=1, clip=True), EMPTY_FLOAT),
        allow_missing=False,
    )
    tau_mass = ak.to_numpy(
        ak.fill_none(ak.pad_none(sorted_taus.mass, 2, axis=1, clip=True), EMPTY_FLOAT),
        allow_missing=False,
    )
    ...
    if inputs_inputs:
        pdf_input_vars_reco_higgs = ak.zip(
            {
                "b1_pt": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_pt[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "b1_eta": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_eta[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "b1_phi": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_phi[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "b1_mass": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_mass[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "b2_pt": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_pt[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "b2_eta": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_eta[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "b2_phi": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_phi[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "b2_mass": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(b_mass[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau1_pt": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_pt[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau1_eta": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_eta[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau1_phi": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_phi[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau1_mass": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_mass[:, 0])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau2_pt": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_pt[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau2_eta": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_eta[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau2_phi": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_phi[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
                "tau2_mass": ak.fill_none(
                    ak.mask(ak.from_numpy(np.ascontiguousarray(tau_mass[:, 1])), ch_id_mask), EMPTY_FLOAT
                ),
            },
        )
    ...
    # inputs: pt, eta, phi, m four bbtautau -> det coords
    inputs = jax.numpy.concatenate(
        [
            b1_corrected,
            b2_corrected,
    ...
    if two_boosts:
        calculate_cos_theta_cms_h1_b1_used = calculate_cos_theta_cms_h1_b1_2boosts
        calculate_cos_theta_cms_h2_tau_vis1_used = calculate_cos_theta_cms_h2_tau_vis1_2boosts
    else:
        calculate_cos_theta_cms_h1_b1_used = calculate_cos_theta_cms_h1_b1
        calculate_cos_theta_cms_h2_tau_vis1_used = calculate_cos_theta_cms_h2_tau_vis1
    ...
    batched_calculate_cos_theta_cms_h2_tau_vis1 = jax.vmap(calculate_cos_theta_cms_h2_tau_vis1_used)
    batched_calculate_phi_cms_h2_tau_vis1 = jax.vmap(calculate_phi_cms_h2_tau_vis1)
    batched_calculate_cos_theta_cms_h1_b1 = jax.vmap(calculate_cos_theta_cms_h1_b1_used)
    ...
    batched_grad_calculate_cos_theta_cms_h2_tau_vis1 = jax.vmap(jax.grad(calculate_cos_theta_cms_h2_tau_vis1_used))
    batched_grad_calculate_phi_cms_h2_tau_vis1 = jax.vmap(jax.grad(calculate_phi_cms_h2_tau_vis1))
    batched_grad_calculate_cos_theta_cms_h1_b1 = jax.vmap(jax.grad(calculate_cos_theta_cms_h1_b1_used))
    ...
    pdf_input_vars_reco_higgs = ak.zip(
        {
            "dihiggs_mass": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(dihiggs_mass, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "dihiggs_system_pt": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(dihiggs_system_pt, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "dihiggs_system_pz": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(dihiggs_system_pz, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "dihiggs_system_phi": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(dihiggs_system_phi, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "cos_theta_h1": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(cos_theta_h1, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "phi_h1": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(phi_h1, nan=EMPTY_FLOAT)), events.channel_id == 3), EMPTY_FLOAT
            ),
            "cos_theta_cms_h2_tau_vis1": ak.fill_none(
                ak.mask(
                    ak.from_numpy(np.nan_to_num(cos_theta_cms_h2_tau_vis1, nan=EMPTY_FLOAT)), events.channel_id == 3
                ),
                EMPTY_FLOAT,
            ),
            "phi_cms_h2_tau_vis1": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(phi_cms_h2_tau_vis1, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "cos_theta_cms_h1_b1": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(cos_theta_cms_h1_b1, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "phi_cms_h1_b1": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(phi_cms_h1_b1, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "constr_term_tau": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(constr_term_tau, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "constr_term_b": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(constr_term_b, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "jac_det": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(jac_det, nan=EMPTY_FLOAT)), events.channel_id == 3), EMPTY_FLOAT
            ),
...
@producer(
    uses={"Tau", "HHBJet", "process_id", "channel_id"},
...
def create_pdf_input_vars_reco_top(
    ...
    **kwargs,
    ...
    # Calculate inputs with helper functions from above, using jax.vmap, then calculate jacobians
    if two_boosts:
        calculate_tau1_cos_theta_star_cms_t1_vis = calculate_tau1_cos_theta_star_cms_t1_vis_2boost
        calculate_tau2_cos_theta_star_cms_t2_vis = calculate_tau2_cos_theta_star_cms_t2_vis_2boost
    else:
        calculate_tau1_cos_theta_star_cms_t1_vis = calculate_tau1_cos_theta_star_cms_t1_vis
        calculate_tau2_cos_theta_star_cms_t2_vis = calculate_tau2_cos_theta_star_cms_t2_vis
    ...
    pdf_input_vars_reco_top = ak.zip(
        {
            "tt_vis_system_mass": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tt_vis_system_mass, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "tt_vis_system_pt": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tt_vis_system_pt, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "tt_vis_system_pz": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tt_vis_system_pz, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "tt_vis_system_phi": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tt_vis_system_phi, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "t_vis_y_diff": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(t_vis_y_diff, nan=EMPTY_FLOAT)), events.channel_id == 3),
                EMPTY_FLOAT,
            ),
            "t1_vis_phi": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(t1_vis_phi, nan=EMPTY_FLOAT)), events.channel_id == 3), EMPTY_FLOAT
            ),
            "tau1_cos_theta_star_cms_t1_vis": ak.fill_none(
                ak.mask(
                    ak.from_numpy(np.nan_to_num(tau1_cos_theta_star_cms_t1_vis, nan=EMPTY_FLOAT)),
                    events.channel_id == 3,
                ),
                EMPTY_FLOAT,
            ),
            "tau1_phi": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tau1_phi, nan=EMPTY_FLOAT)), events.channel_id == 3), EMPTY_FLOAT
            ),
            "tau2_cos_theta_star_cms_t2_vis": ak.fill_none(
                ak.mask(
                    ak.from_numpy(np.nan_to_num(tau2_cos_theta_star_cms_t2_vis, nan=EMPTY_FLOAT)),
                    events.channel_id == 3,
                ),
                EMPTY_FLOAT,
            ),
            "tau2_phi": ak.fill_none(
                ak.mask(ak.from_numpy(np.nan_to_num(tau2_phi, nan=EMPTY_FLOAT)), events.channel_id == 3), EMPTY_FLOAT
            ),
            "tau1_cos_theta_star_cms_wplus": ak.fill_none(
                ak.mask(
                    ak.from_numpy(np.nan_to_num(tau1_cos_theta_star_cms_wplus, nan=EMPTY_FLOAT)), events.channel_id == 3
                ),
                EMPTY_FLOAT,
            ),
            "tau2_cos_theta_star_cms_wminus": ak.fill_none(
                ak.mask(
                    ak.from_numpy(np.nan_to_num(tau2_cos_theta_star_cms_wminus, nan=EMPTY_FLOAT)),
                    events.channel_id == 3,
                ),
                EMPTY_FLOAT,
            ),
...
@producer(
    uses={
        create_pdf_input_vars_reco_top,
        create_pdf_input_vars_reco_higgs,
    },
    produces={
        create_pdf_input_vars_reco_top,
        create_pdf_input_vars_reco_higgs,
...
def pdf_inputs(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    events = self[create_pdf_input_vars_reco_top](events, nn_outputs=True, **kwargs)
    events = self[create_pdf_input_vars_reco_higgs](events, nn_outputs=True, **kwargs)
    return events
...
def pdf_inputs_inputs(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    events = self[create_pdf_input_vars_reco_higgs](events, nn_outputs=True, inputs_inputs=True, **kwargs)
    events = self[create_pdf_input_vars_reco_top](events, nn_outputs=True, inputs_inputs=True, **kwargs)
    return events
