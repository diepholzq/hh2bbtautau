...
def add_field(muons, electrons, nu_mu, nu_e, field: str):
    return ak.fill_none(muons[field][:, 0], 0) + ak.fill_none(electrons[field][:, 0], 0) + ak.fill_none(
        nu_mu[field][:, 0], 0) + ak.fill_none(nu_e[field][:, 0], 0)
...
def higgs_decay_products(self: Producer, events: ak.Array, **kwargs) -> ak.Array:
    ...
    #     "check_attr": "metric_table", "skip_fields": "*Idx*G"}}).sum().absolute()
    # taus_inv_mass = attach_coffea_behavior_fn(taus, collections={"higgs_family": {"type_name": "GenParticle",
    #     "check_attr": "metric_table", "skip_fields": "*Idx*G"}}).sum().absolute()
