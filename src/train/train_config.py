...
LOSS_CHOICE = Literal[
    "cross_entropy", "signal_efficiency", "signal_efficiency_binning_aware"
]
...
class DataConfig:
    ...
    target_map: Dict[str, int] = field(
        # default_factory=lambda: {"hh": 0, "tt": 1},
        default_factory=lambda: {"hh": 0, "tt": 1, "dy": 2}
    )  # node: index
    ...
    eras: Tuple[ERAS_CHOICE] = "22post"
    datasets: Optional[List[str]] = None
    cuts: Optional[Any] = None

    def __post_init__(self):
        # a dictionary of all files corresponding to a certain dataset
        self.datasets = load_data.find_datasets(
            self.dataset_pattern, self.eras, file_type="root", verbose=False
        )
...
class ModelBuildingConfig:
    ...
    # Padding Layers
    categorical_padding_value: Optional[float] = (
        None  # missing values in categorical inputs are replaced by this
    )
    continuous_padding_value: Optional[float] = (
        -4  # missing values in continuous inputs are replaced by this
    )

    # DenseNet/LBN config
    nodes: int = (
        128  # base number of nodes of the dense blocks, due to DenseNet connection, increases with more layers
    )
    activation_functions: str = (
        "elu"  # string of the activation function of DenseBlocks - Marcel DNN: elu
    )
    ...
    batch_norm_eps: float = (
        0.001  # epsilon denominator of batch norm - increase stability Marcel: 0.001
    )
    LBN_M: int = 10  # number of particles of the lbn network Marcel: 10
    last_activation_fn: LAST_ACTIVATION_CHOICE = (
        "Softmax"  # add activation function after last layer
    )
    use_last_activation: bool = (
        True  # whether to use the last activation function, can be deactivated if not wanted - for example when using a loss function that already includes an activation like cross entropy, Marcel: False
    )
    # STD Layers
    ...
    def __post_init__(self):
        ...
        if self.normalize_linear is True:
            raise ValueError(
                "Norm of Linear Layer is currently buggy and is therefore disabled for now"
            )
...
class TrainingConfig:
    ...
    model_choice: MODEL_CHOICE = "dense"
    loss_fn: LOSS_CHOICE = "cross_entropy"
    training_fn: TRAINING_LOOP_CHOICE = "cross_entropy"  # name of the training loop
    validation_fn: VALIDATION_LOOP_CHOICE = (
        "cross_entropy"  # name of the validation loop
    )
    loss_mode: SIGNAL_EFFICIENCY_LOSS_MODE = (
        "no_unc"  # only relevant if signal_efficiency loss is chosen, determines which formula is used for the asimov calculation
    )
    loss_uncertainty: float = (
        0.0  # # only relevant if signal_efficiency loss is chosen, determines the background uncertainty used in the asimov calculation
    )
    ...
    validation_interval: int = (
        100  # interval between two validation passes / plots are done during validation
    )
    gamma: float = 0.5
    ...
    sub_process_ratios: Dict[str, float] = field(
        default_factory=lambda: {  # decide the
            ...
            "tt": {
                (1100, 1200): 1,
                1300: 1,
            },  # groups are possible, and mixes are allowed
            "dy": {
                51667: 1,
                51683: 1,
                51664: 1,
    ...
    min_events_in_batch: int = (
        1  # sampler setting: minimal number of events of a subprocess in a batch
    )
    ...
    def __post_init__(self):
        ...
        self.sub_process_ratios = multiply_sub_process_rates(
            self.use_sub_process_ratios, self.sub_process_ratios
        )


@dataclass
class SchedulerConfig:
    scheduler_chain: Tuple[SCHEDULER_CHOICE, ...] = (
        "linear",
        "cosine_annealing",
    )  # schedulers used in chain
    ...
    milestones: Tuple[int, ...] = (
        500,
    )  # intervals after which the LR scheduler is swapped
    ...
    class ReduceLROnPlateauConfig:
        mode: str = (
            "min"  # one of {min, max}, in min mode lr will be reduced when the quantity monitored has stopped decreasing
        )
        ...
        cooldown: int = (
            0,
        )  # number of iterations to wait after a learning rate reduction before resuming normal operation
        min_lr: float = (0,)  # lower bound on the learning rate
        eps: float = (
            1e-08,
        )  # minimal decay applied to lr, if it is smaller than this value, it is set to this value

    @dataclass
    class LinearLRConfig:
        start_factor: float = (
            0.01  # the initial learning rate will be the start_factor times the base learning rate
        )
        end_factor: float = (
            1.0  # the final learning rate will be the end_factor times the base learning rate
        )
        total_iters: int = (
    ...
    def __post_init__(self):
        ...
        scheduler_register = {
            "cosine_annealing": (
                self.CosineAnnealingLRConfig,
                schedulers.CosineAnnealingLR,
            ),
            "reduce_on_plateau": (
                self.ReduceLROnPlateauConfig,
...
@dataclass
class DebugConfig:
    get_batch_statistic_return_dummy: bool = (
        False  # do not calculate preprocessing, instead use mean = 0, std = 1
    )
    load_marcel_stats: bool = (
        False  # load Marcels mean and std from preprocessing into STD Layer
    )
...
class FullConfig:
    ...
    model_building_config: ModelBuildingConfig = field(
        default_factory=ModelBuildingConfig
    )
    ...
    def __post_init__(self):
        ...
        if (self.training_config.training_fn != "sam") & (
