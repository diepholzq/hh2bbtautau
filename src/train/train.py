...
def main(**kwargs):
    ...
    for current_fold in full_config.training_config.train_folds:
        ...
        train_events, validation_events = fold_split_coordinator(
            events, which="training"
        ), fold_split_coordinator(events, which="validation")
        weight_aggregator = preprocessing.WeightAggregator(
            events, fold_split_coordinator.indices
        )
        ...
        (
            full_config.model_building_config.mean,
        ...
        training_loop, validation_loop = TrainingLoop(full_config), ValidationLoop(
            full_config
        )
        ...
        for current_iteration in range(1_000_000):
            ...
            if current_iteration % full_config.training_config.verbose_interval == 0:
                tensorboard_writer.log_loss(
                    {"batch_loss": t_loss.item()}, step=current_iteration
                )
                current_lr = optimizer_inst.param_groups[0]["lr"]
                logger_inst.training(
                    f"T-It: {current_iteration} - LR: {current_lr} - batch loss: {t_loss.item():.2E}"
                )
            # ----
            #### Evaluation of training and validation data, logging and checkpointing
            # ----
            if (
                current_iteration % full_config.training_config.validation_interval == 0
            ) & (current_iteration >= 0):
                ...
                eval_t_loss, (eval_t_pred, eval_t_tar, eval_t_weights) = (
                    validation_loop(
                        model_inst,
                ...
                eval_v_loss, (eval_v_pred, eval_v_tar, eval_v_weights) = (
                    validation_loop(
                        model_inst,
                        validation_loss_inst,
                        validation_sampler,
                ...
                if isinstance(
                    ...
                    if previous_lr != current_lr:
                        ...
                        model_inst.load_state_dict(
                            checkpoint_inst.last_checkpoint["model_state_dict"]
                        )
